...
//$content = '<a class="link_image" href="{{ post_link_url }}" title="{{ post_title }}">{{ post_title }}</a>';
if ( ! empty( $link_html ) ) {
...
if($font_container_data['values']['tag']=='h4'){
    $output .= '<a class="link_image" href="{{ post_link_url }}" title="{{ post_title }}">{{ post_title }}</a>';
}
else{
    $output .= $content;
}
